...
send(Req) ->
    ...
    case valid_token(Req) of
        ...
        true ->
            ...
            CheckPids = case UserId of
                "all" ->
                    emysql:execute(myjqrealtime, lists:concat(["SELECT * FROM (SELECT * FROM `processes` ORDER BY end_at DESC) as Sub GROUP BY Sub.browser_session"]));
                _ ->
                    emysql:execute(myjqrealtime, lists:concat(["SELECT * FROM (SELECT * FROM `processes` WHERE user_id = ", emysql_util:quote(UserId), " ORDER BY end_at DESC) as Sub GROUP BY Sub.browser_session"]))
            end,
